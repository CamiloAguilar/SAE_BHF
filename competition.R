...
Gr3 <- c("CALENDARIO", "NATURALEZA", "JORNADA", "EVALUADOS") 

# Otras; todas las demás
Gr4 <- c("FINS_PERSONASHOGARACTUAL", "FINS_CUARTOSHOGARACTUAL", "FINS_PISOSHOGAR", "FINS_TIENEINTERNET",
         "FINS_TIENECOMPUTADOR", "FINS_TIENEAUTOMOVILPARTICULAR", "INGLES_DESEM", "PERS_GENERO")

# Generamos lista con las variables a iterar
gr_vars <- list(Obj, Gr1, Gr2, Gr3, Gr4)
names(gr_vars) <- c("Obj", "Gr1", "Gr2", "Gr3", "Gr4")
...
summary(modelos)
tabla_ajuste <- modelos$modelo_1$mean_cve
for(i in 2:length(modelos)){
  tabla_ajuste <- rbind(tabla_ajuste, modelos[[i]]$mean_cve)
}
row.names(tabla_ajuste) <- NULL
tabla_ajuste <- as.data.frame(tabla_ajuste)
colnames(tabla_ajuste) <- c("Var1", "Var2", "Var3", "Var4", "mean_cve", "mean_mse")
tabla_ajuste
